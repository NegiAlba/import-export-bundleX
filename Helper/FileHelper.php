...
class FileHelper
{
    const PATTERN_TIMESTAMPED_FILE = '/^(\d{4}-\d{2}-\d{2}-\d{2}-\d{2}-\d{2})_%s$/';
    ...
    public static function getWrongTimeStampedFiles($path, $filename)
    {
        ...
        foreach ($matchesFiles as $file) {
            ...
            if (count($matches) === 0 && $pathParts['basename'] !== $filename) {
                $wrongFiles[] = $file;
            }
        }
    ...
    public static function getCurrentTimeStampedZipAssetFile($type, $csvFile)
    {
        ...
        $timestampedZipFilename = $pathParts['dirname'] . DIRECTORY_SEPARATOR . $pathParts['filename'] . '.zip';
        if (file_exists($timestampedZipFilename)) {
            ...
            $assetFile = $pathParts['dirname'] . DIRECTORY_SEPARATOR . $type . '.zip';
        }
        return $assetFile;
    }
}
